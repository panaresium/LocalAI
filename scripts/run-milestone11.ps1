...
$checks = @(
    @{ name = "milestone10-regression"; command = "powershell"; args = @("-NoProfile", "-ExecutionPolicy", "Bypass", "-File", "scripts\run-milestone10.ps1") },
    @{ name = "desktop-build"; command = $pnpmRunner.FilePath; args = @($pnpmRunner.ArgsPrefix + @("--filter", "@hermes-local-ai/studio-desktop", "build")) },
    @{ name = "media-system"; command = "powershell"; args = @("-NoProfile", "-ExecutionPolicy", "Bypass", "-File", "scripts\test-media-system.ps1") },
    @{ name = "milestone11-node-tests"; command = "node"; args = @("--test", "tests\milestone11\*.test.mjs") }
)
