...
$checks = @(
    @{ name = "milestone6-regression"; command = "powershell"; args = @("-NoProfile", "-ExecutionPolicy", "Bypass", "-File", "scripts\run-milestone6.ps1") },
    @{ name = "windows-broker-observe"; command = "powershell"; args = @("-NoProfile", "-ExecutionPolicy", "Bypass", "-File", "scripts\test-windows-broker-observe.ps1") },
    @{ name = "desktop-build"; command = $pnpmRunner.FilePath; args = @($pnpmRunner.ArgsPrefix + @("--filter", "@hermes-local-ai/studio-desktop", "build")) },
    @{ name = "milestone7-node-tests"; command = "node"; args = @("--test", "tests\milestone7\*.test.mjs") }
)
