...
function Limit-CapturedOutput {
    ...
    return $Output.Substring(0, $headLength) + "`n... output truncated by run-milestone16.ps1 ...`n" + $Output.Substring($Output.Length - $tailLength)
}

$checks = @(
    @{ name = "milestone15-regression"; command = "powershell"; args = @("-NoProfile", "-ExecutionPolicy", "Bypass", "-File", "scripts\run-milestone15.ps1") },
    @{ name = "desktop-build"; command = $pnpmRunner.FilePath; args = @($pnpmRunner.ArgsPrefix + @("--filter", "@hermes-local-ai/studio-desktop", "build")) },
    @{ name = "packaging-hardening"; command = "powershell"; args = @("-NoProfile", "-ExecutionPolicy", "Bypass", "-File", "scripts\test-packaging-hardening.ps1") },
    @{ name = "milestone16-node-tests"; command = "node"; args = @("--test", "tests\milestone16\*.test.mjs") }
)
