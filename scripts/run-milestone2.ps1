...
$Artifacts = Join-Path $Root "artifacts\milestone2"
...
$checks = @(
    @{ name = "milestone1-regression"; command = "powershell"; args = @("-NoProfile", "-ExecutionPolicy", "Bypass", "-File", "scripts\run-milestone1.ps1") },
    @{ name = "desktop-build"; command = $pnpmRunner.FilePath; args = @($pnpmRunner.ArgsPrefix + @("--filter", "@hermes-local-ai/studio-desktop", "build")) },
    @{ name = "milestone2-node-tests"; command = "node"; args = @("--test", "tests\milestone2\*.test.mjs") }
)
