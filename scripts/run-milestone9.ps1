...
$checks = @(
    @{ name = "milestone8-regression"; command = "powershell"; args = @("-NoProfile", "-ExecutionPolicy", "Bypass", "-File", "scripts\run-milestone8.ps1") },
    @{ name = "browser-vision"; command = "powershell"; args = @("-NoProfile", "-ExecutionPolicy", "Bypass", "-File", "scripts\test-browser-vision.ps1") },
    @{ name = "desktop-build"; command = $pnpmRunner.FilePath; args = @($pnpmRunner.ArgsPrefix + @("--filter", "@hermes-local-ai/studio-desktop", "build")) },
    @{ name = "milestone9-node-tests"; command = "node"; args = @("--test", "tests\milestone9\*.test.mjs") }
)
