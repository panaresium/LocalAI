...
$checks = @(
    @{ name = "milestone12-regression"; command = "powershell"; args = @("-NoProfile", "-ExecutionPolicy", "Bypass", "-File", "scripts\run-milestone12.ps1") },
    @{ name = "desktop-build"; command = $pnpmRunner.FilePath; args = @($pnpmRunner.ArgsPrefix + @("--filter", "@hermes-local-ai/studio-desktop", "build")) },
    @{ name = "app-adapters"; command = "powershell"; args = @("-NoProfile", "-ExecutionPolicy", "Bypass", "-File", "scripts\test-app-adapters.ps1") },
    @{ name = "milestone13-node-tests"; command = "node"; args = @("--test", "tests\milestone13\*.test.mjs") }
)
