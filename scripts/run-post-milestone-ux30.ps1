...
function Limit-CapturedOutput {
    ...
    return $Output.Substring(0, $headLength) + "`n... output truncated by run-post-milestone-ux30.ps1 ...`n" + $Output.Substring($Output.Length - $tailLength)
}

$checks = @(
    @{ name = "post-milestone-ux29-regression"; command = "powershell"; args = @("-NoProfile", "-ExecutionPolicy", "Bypass", "-File", "scripts\run-post-milestone-ux29.ps1") },
    @{ name = "desktop-build"; command = $pnpmRunner.FilePath; args = @($pnpmRunner.ArgsPrefix + @("--filter", "@hermes-local-ai/studio-desktop", "build")) },
    @{ name = "command-review-queue-summary"; command = "powershell"; args = @("-NoProfile", "-ExecutionPolicy", "Bypass", "-File", "scripts\test-command-review-queue-summary.ps1") },
    @{ name = "command-review-queue-summary-node-tests"; command = "node"; args = @("--test", "tests\post-milestone-ux\*.test.mjs") }
)
