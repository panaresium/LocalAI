...
$checks = @(
    @{ name = "milestone11-regression"; command = "powershell"; args = @("-NoProfile", "-ExecutionPolicy", "Bypass", "-File", "scripts\run-milestone11.ps1") },
    @{ name = "desktop-build"; command = $pnpmRunner.FilePath; args = @($pnpmRunner.ArgsPrefix + @("--filter", "@hermes-local-ai/studio-desktop", "build")) },
    @{ name = "teach-mode"; command = "powershell"; args = @("-NoProfile", "-ExecutionPolicy", "Bypass", "-File", "scripts\test-teach-mode.ps1") },
    @{ name = "milestone12-node-tests"; command = "node"; args = @("--test", "tests\milestone12\*.test.mjs") }
)
