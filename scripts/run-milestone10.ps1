...
$checks = @(
    @{ name = "milestone9-regression"; command = "powershell"; args = @("-NoProfile", "-ExecutionPolicy", "Bypass", "-File", "scripts\run-milestone9.ps1") },
    @{ name = "desktop-build"; command = $pnpmRunner.FilePath; args = @($pnpmRunner.ArgsPrefix + @("--filter", "@hermes-local-ai/studio-desktop", "build")) },
    @{ name = "voice-system"; command = "powershell"; args = @("-NoProfile", "-ExecutionPolicy", "Bypass", "-File", "scripts\test-voice-system.ps1") },
    @{ name = "milestone10-node-tests"; command = "node"; args = @("--test", "tests\milestone10\*.test.mjs") }
)
