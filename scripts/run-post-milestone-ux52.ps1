...
$Artifacts = Join-Path $Root "artifacts\post-milestone-ux52"
...
function Limit-CapturedOutput {
    ...
    return $Output.Substring(0, $headLength) + "`n... output truncated by run-post-milestone-ux52.ps1 ...`n" + $Output.Substring($Output.Length - $tailLength)
}

$checks = @(
    @{ name = "desktop-build"; command = $pnpmRunner.FilePath; args = @($pnpmRunner.ArgsPrefix + @("--filter", "@hermes-local-ai/studio-desktop", "build")) },
    @{ name = "model-marketplace-latest-june2026"; command = "powershell"; args = @("-NoProfile", "-ExecutionPolicy", "Bypass", "-File", "scripts\test-model-marketplace-latest-june2026.ps1") },
    @{ name = "post-milestone-ux-node-regression"; command = "node"; args = @("--test", "tests\post-milestone-ux\*.test.mjs") }
)
