...
$Artifacts = Join-Path $Root "artifacts\milestone3"
...
$checks = @(
    @{ name = "milestone2-regression"; command = "powershell"; args = @("-NoProfile", "-ExecutionPolicy", "Bypass", "-File", "scripts\run-milestone2.ps1") },
    @{ name = "desktop-build"; command = $pnpmRunner.FilePath; args = @($pnpmRunner.ArgsPrefix + @("--filter", "@hermes-local-ai/studio-desktop", "build")) },
    @{ name = "milestone3-node-tests"; command = "node"; args = @("--test", "tests\milestone3\*.test.mjs") }
)
