...
$Artifacts = Join-Path $Root "artifacts\post-milestone-ux35"
...
function Limit-CapturedOutput {
    ...
    return $Output.Substring(0, $headLength) + "`n... output truncated by run-post-milestone-ux35.ps1 ...`n" + $Output.Substring($Output.Length - $tailLength)
}

$checks = @(
    @{ name = "post-milestone-ux34-regression"; command = "powershell"; args = @("-NoProfile", "-ExecutionPolicy", "Bypass", "-File", "scripts\run-post-milestone-ux34.ps1") },
    @{ name = "desktop-build"; command = $pnpmRunner.FilePath; args = @($pnpmRunner.ArgsPrefix + @("--filter", "@hermes-local-ai/studio-desktop", "build")) },
    @{ name = "command-review-timeline"; command = "powershell"; args = @("-NoProfile", "-ExecutionPolicy", "Bypass", "-File", "scripts\test-command-review-timeline.ps1") },
    @{ name = "command-review-timeline-node-tests"; command = "node"; args = @("--test", "tests\post-milestone-ux\*.test.mjs") }
)
