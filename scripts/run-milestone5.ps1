...
$Artifacts = Join-Path $Root "artifacts\milestone5"
...
$checks = @(
    @{ name = "milestone4-regression"; command = "powershell"; args = @("-NoProfile", "-ExecutionPolicy", "Bypass", "-File", "scripts\run-milestone4.ps1") },
    @{ name = "desktop-build"; command = $pnpmRunner.FilePath; args = @($pnpmRunner.ArgsPrefix + @("--filter", "@hermes-local-ai/studio-desktop", "build")) },
    @{ name = "milestone5-node-tests"; command = "node"; args = @("--test", "tests\milestone5\*.test.mjs") }
)
