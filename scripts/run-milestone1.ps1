...
$Artifacts = Join-Path $Root "artifacts\milestone1"
...
$checks = @(
    @{ name = "milestone0-regression"; command = "powershell"; args = @("-NoProfile", "-ExecutionPolicy", "Bypass", "-File", "scripts\run-milestone0.ps1") },
    @{ name = "desktop-build"; command = $pnpmRunner.FilePath; args = @($pnpmRunner.ArgsPrefix + @("--filter", "@hermes-local-ai/studio-desktop", "build")) },
    @{ name = "milestone1-node-tests"; command = "node"; args = @("--test", "tests\milestone1\*.test.mjs") }
)
