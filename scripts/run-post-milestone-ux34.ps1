...
function Limit-CapturedOutput {
    ...
    return $Output.Substring(0, $headLength) + "`n... output truncated by run-post-milestone-ux34.ps1 ...`n" + $Output.Substring($Output.Length - $tailLength)
}

$checks = @(
    @{ name = "post-milestone-ux33-regression"; command = "powershell"; args = @("-NoProfile", "-ExecutionPolicy", "Bypass", "-File", "scripts\run-post-milestone-ux33.ps1") },
    @{ name = "desktop-build"; command = $pnpmRunner.FilePath; args = @($pnpmRunner.ArgsPrefix + @("--filter", "@hermes-local-ai/studio-desktop", "build")) },
    @{ name = "command-focus-actions"; command = "powershell"; args = @("-NoProfile", "-ExecutionPolicy", "Bypass", "-File", "scripts\test-command-focus-actions.ps1") },
    @{ name = "command-focus-actions-node-tests"; command = "node"; args = @("--test", "tests\post-milestone-ux\*.test.mjs") }
)
