...
$Artifacts = Join-Path $Root "artifacts\milestone4"
...
$checks = @(
    @{ name = "milestone3-regression"; command = "powershell"; args = @("-NoProfile", "-ExecutionPolicy", "Bypass", "-File", "scripts\run-milestone3.ps1") },
    @{ name = "desktop-build"; command = $pnpmRunner.FilePath; args = @($pnpmRunner.ArgsPrefix + @("--filter", "@hermes-local-ai/studio-desktop", "build")) },
    @{ name = "milestone4-node-tests"; command = "node"; args = @("--test", "tests\milestone4\*.test.mjs") }
)
