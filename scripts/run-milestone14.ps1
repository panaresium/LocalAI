...
$checks = @(
    @{ name = "milestone13-regression"; command = "powershell"; args = @("-NoProfile", "-ExecutionPolicy", "Bypass", "-File", "scripts\run-milestone13.ps1") },
    @{ name = "desktop-build"; command = $pnpmRunner.FilePath; args = @($pnpmRunner.ArgsPrefix + @("--filter", "@hermes-local-ai/studio-desktop", "build")) },
    @{ name = "elevated-helper"; command = "powershell"; args = @("-NoProfile", "-ExecutionPolicy", "Bypass", "-File", "scripts\test-elevated-helper.ps1") },
    @{ name = "milestone14-node-tests"; command = "node"; args = @("--test", "tests\milestone14\*.test.mjs") }
)
