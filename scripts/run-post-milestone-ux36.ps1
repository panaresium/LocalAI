...
$Artifacts = Join-Path $Root "artifacts\post-milestone-ux36"
...
function Limit-CapturedOutput {
    ...
    return $Output.Substring(0, $headLength) + "`n... output truncated by run-post-milestone-ux36.ps1 ...`n" + $Output.Substring($Output.Length - $tailLength)
}

$checks = @(
    @{ name = "post-milestone-ux35-regression"; command = "powershell"; args = @("-NoProfile", "-ExecutionPolicy", "Bypass", "-File", "scripts\run-post-milestone-ux35.ps1") },
    @{ name = "desktop-build"; command = $pnpmRunner.FilePath; args = @($pnpmRunner.ArgsPrefix + @("--filter", "@hermes-local-ai/studio-desktop", "build")) },
    @{ name = "studio-header-context"; command = "powershell"; args = @("-NoProfile", "-ExecutionPolicy", "Bypass", "-File", "scripts\test-studio-header-context.ps1") },
    @{ name = "studio-header-context-node-tests"; command = "node"; args = @("--test", "tests\post-milestone-ux\*.test.mjs") }
)
