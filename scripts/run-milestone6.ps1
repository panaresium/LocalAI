...
$Artifacts = Join-Path $Root "artifacts\milestone6"
...
$checks = @(
    @{ name = "milestone5-regression"; command = "powershell"; args = @("-NoProfile", "-ExecutionPolicy", "Bypass", "-File", "scripts\run-milestone5.ps1") },
    @{ name = "desktop-build"; command = $pnpmRunner.FilePath; args = @($pnpmRunner.ArgsPrefix + @("--filter", "@hermes-local-ai/studio-desktop", "build")) },
    @{ name = "milestone6-node-tests"; command = "node"; args = @("--test", "tests\milestone6\*.test.mjs") }
)
