...
$Artifacts = Join-Path $Root "artifacts\post-milestone-ux47"
...
function Limit-CapturedOutput {
    ...
    return $Output.Substring(0, $headLength) + "`n... output truncated by run-post-milestone-ux47.ps1 ...`n" + $Output.Substring($Output.Length - $tailLength)
}

$checks = @(
    @{ name = "desktop-build"; command = $pnpmRunner.FilePath; args = @($pnpmRunner.ArgsPrefix + @("--filter", "@hermes-local-ai/studio-desktop", "build")) },
    @{ name = "chat-plan-duplicate-guard"; command = "powershell"; args = @("-NoProfile", "-ExecutionPolicy", "Bypass", "-File", "scripts\test-chat-plan-duplicate-guard.ps1") },
